...
int hall_switch_init(const char *uart_device)
{
    ...
    // 初始化ADS122U04寄存器（根据寄存器定义配置）
    // 业务需求：手动读取(AUTO=0) + CRC16 + DCNT
    g_hall_ctx.auto_enabled = false;
    g_hall_ctx.dcnt_enabled = false;
    g_hall_ctx.last_dcnt = g_hall_ctx.dcnt_enabled ? 0xFF : 0;
    // CONFIG0: MUX=AIN3-AVSS, GAIN=1, PGA_BYPASS=DISABLED (0xB1)
    uint8_t config0 = (uint8_t)(ADS122U04_CONFIG0_MUX_AIN3_AVSS | 
                                 ADS122U04_CONFIG0_GAIN_1 | 
                                 ADS122U04_CONFIG0_PGA_BYPASS_DISABLED);
    ret = hall_reg_write(ADS122U04_REG_CONFIG0, config0);
    if (ret < 0)
    {
        log_err("Failed to configure ADS122U04 CONFIG0\n");
        close(g_hall_ctx.uart_fd);
        g_hall_ctx.uart_fd = -1;
        return -1;
    }
    ...
    // 读回验证CONFIG0
    uint8_t read_config0 = 0;
    ret = hall_reg_read(ADS122U04_REG_CONFIG0, &read_config0);
    if (ret < 0 || read_config0 != config0)
    {
        log_err("CONFIG0 verify failed: wrote=0x%02X, read=0x%02X\n", config0, read_config0);
        close(g_hall_ctx.uart_fd);
        g_hall_ctx.uart_fd = -1;
        return -1;
    }
    log_info("CONFIG0 verified: 0x%02X\n", read_config0);
    ...
    // 读回验证CONFIG2
    uint8_t read_config2 = 0;
    ret = hall_reg_read(ADS122U04_REG_CONFIG2, &read_config2);
    if (ret < 0 || read_config2 != config2)
    {
        log_err("CONFIG2 verify failed: wrote=0x%02X, read=0x%02X\n", config2, read_config2);
        close(g_hall_ctx.uart_fd);
        g_hall_ctx.uart_fd = -1;
        return -1;
    }
    log_info("CONFIG2 verified: 0x%02X\n", read_config2);

    // 以硬件读回为准同步驱动侧状态，避免DCNT/CRC状态不一致导致解析失败或误判不更新
    g_hall_ctx.dcnt_enabled = ((read_config2 & ADS122U04_CONFIG2_DCNT_MASK) != 0);
    g_hall_ctx.crc_enabled = (((read_config2 & ADS122U04_CONFIG2_CRC_MASK) >> ADS122U04_CONFIG2_CRC_SHIFT) != 0);
    g_hall_ctx.last_dcnt = g_hall_ctx.dcnt_enabled ? 0xFF : 0;
    
    log_info("CONFIG2 sync: dcnt_enabled=%d crc_enabled=%d last_dcnt=%d\n",
             g_hall_ctx.dcnt_enabled, g_hall_ctx.crc_enabled, g_hall_ctx.last_dcnt);
    ...
    ret = hall_reg_write(ADS122U04_REG_CONFIG4, config4);
    if (ret < 0)
    {
        log_err("Failed to configure ADS122U04 CONFIG4\n");
        close(g_hall_ctx.uart_fd);
        g_hall_ctx.uart_fd = -1;
        return -1;
    }
    ...
    log_info("CONFIG4 verified: 0x%02X\n", read_config4);

    // 手动读取模式下仍建议开启连续转换，提高读取实时性
    (void)ads_send_cmd(ADS_CMD_START);
    tcflush(g_hall_ctx.uart_fd, TCIFLUSH);

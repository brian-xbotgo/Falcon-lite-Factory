...
if [ -f "${TARGET_DIR}/etc/fstab" ]; then
    sed -i '/PARTLABEL=oem/d' "${TARGET_DIR}/etc/fstab"
    echo ">>> Forced removal of PARTLABEL=oem from /etc/fstab"
    sed -i '/^\/dev\/root/s/rw/ro/' "${TARGET_DIR}/etc/fstab"
    sed -i '/^PARTLABEL=device_data/s/defaults/ro/' "${TARGET_DIR}/etc/fstab"
fi

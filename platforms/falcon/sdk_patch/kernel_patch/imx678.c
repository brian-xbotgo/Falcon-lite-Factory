...
#include "../../video/rockchip/vehicle/vehicle_cfg.h"
				...
				struct v4l2_mbus_config *config)
{
    ...
    config->bus.mipi_csi2.num_data_lanes = imx678->lane_num;
	//config->flags = val;

	return 0;
}

static void imx678_get_otp(struct otp_info *otp, struct rkmodule_inf *inf)
{
    printk( "[IMX678 OTP] Enter imx678_get_otp, otp addr: %pK, inf addr: %pK\n",
           otp, inf);

    if (!otp || !inf) {
        printk("[IMX678 OTP] Error: otp or inf is NULL!\n");
        return;
    }

    printk("[IMX678 OTP] AWB flag: %d\n", otp->awb_data.flag);
    if (otp->awb_data.flag) {
        ...
        printk("[IMX678 OTP] AWB data loaded successfully:\n");
        printk("  r_value: 0x%x, b_value: 0x%x, gr_value: 0x%x, gb_value: 0x%x\n",
               inf->awb.r_value, inf->awb.b_value, inf->awb.gr_value, inf->awb.gb_value);
        printk("  golden_r: 0x%x, golden_b: 0x%x, golden_gr: 0x%x, golden_gb: 0x%x\n",
               inf->awb.golden_r_value, inf->awb.golden_b_value,
               inf->awb.golden_gr_value, inf->awb.golden_gb_value);
    } else {
        printk("[IMX678 OTP] AWB data not available (flag=0)\n");
        inf->awb.flag = 0;
    }


    printk("[IMX678 OTP] LSC flag: %d\n", otp->lsc_data.flag);
    if (otp->lsc_data.flag) {
        ...
        /* 打印 LSC 基础信息 */
        printk("[IMX678 OTP] LSC data loaded successfully:\n");
        printk("  resolution: %dx%d, table_size: %d\n",
               inf->lsc.width, inf->lsc.height, inf->lsc.table_size);
        printk("  LSC data buffer addr: %pK, total length: %zu bytes\n",
               otp->lsc_data.data, otp->lsc_data.table_size * sizeof(uint8_t));

        printk("[IMX678 OTP] LSC table sample data (first 3 + last 2 entries):\n");
        for (int i = 0; i < 289; i++) {
            ...
            /* 打印关键位置的样本数据 */
            if (i < 3 || i >= 287) {
                printk("  index[%d]: R=0x%x, GR=0x%x, GB=0x%x, B=0x%x\n",
                       i, inf->lsc.lsc_r[i], inf->lsc.lsc_gr[i],
                       inf->lsc.lsc_gb[i], inf->lsc.lsc_b[i]);
            } else if (i == 3) {
                printk("  ... (skip middle %d entries) ...\n", 289 - 5);
            }
        }
        printk("[IMX678 OTP] LSC table load complete. Last entry:\n");
        printk("  index[288]: R=0x%x, GR=0x%x, GB=0x%x, B=0x%x\n",
               inf->lsc.lsc_r[288], inf->lsc.lsc_gr[288],
               inf->lsc.lsc_gb[288], inf->lsc.lsc_b[288]);
    } else {
        printk("[IMX678 OTP] LSC data not available (flag=0)\n");
        inf->lsc.flag = 0;
    }

    /* 函数退出打印 */
    printk("[IMX678 OTP] Exit imx678_get_otp, OTP data processing done\n");
}

static void imx678_get_module_inf(struct imx678 *imx678,
				  struct rkmodule_inf *inf)
{
	struct otp_info *otp = imx678->otp;
	...
	if (otp) {
		imx678_get_otp(otp, inf);
	} else {
		printk("[IMX678 OTP] imx678_get_module_inf is otp error\n");
	}	
}
...
static int imx678_write_shr0(struct imx678 *imx678, u32 shr0)
{
	int ret = 0;
...
static int imx678_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ANALOGUE_GAIN:
		...
		break;
	case V4L2_CID_VBLANK: {
		u32 prev_vts = imx678->cur_vts;

		target_vts = ctrl->val + imx678->cur_mode->height;
		...
		if (imx678->cur_mode->hdr_mode == NO_HDR) {
			exp_lines = imx678->exposure->val;
			if (exp_lines > target_vts - 8)
				exp_lines = target_vts - 8;
			shr0 = target_vts - exp_lines;

			if (!imx678->pclk)
				imx678_get_pclk_and_tline(imx678);

			if (target_vts > prev_vts) {
				/*
				 * VTS increasing (e.g. 30fps->25fps):
				 * New SHR0 may be >= old VTS, which is illegal — sensor
				 * ignores the write and keeps old SHR0. When VTS then grows,
				 * exposure = new_VTS - old_SHR0 becomes very large → white flash.
				 *
				 * Fix: write VTS first (N+1), wait one old frame so VTS takes
				 * effect, then write SHR0 (N+2 from that point). The one
				 * intermediate frame uses old SHR0 which is always < old VTS <
				 * new VTS, so it is legal and exposure is only slightly longer.
				 */
				frame_us = (u32)div_u64((u64)imx678->cur_mode->hts_def *
							prev_vts * 1000000,
							imx678->pclk);
				if (frame_us < 1000)
					frame_us = 1000;
				...
				usleep_range(frame_us, frame_us + 200);
				ret |= imx678_write_shr0(imx678, shr0);
				dev_dbg(&client->dev, "vts increase: vts=%d shr0=%d\n", vts, shr0);
				break;
			} else {
				/*
				 * VTS decreasing (e.g. 25fps->30fps):
				 * Write SHR0 first (N+2), delay one frame, then write VTS (N+1).
				 */
				ret = imx678_write_shr0(imx678, shr0);
				if (!ret) {
					frame_us = (u32)div_u64((u64)imx678->cur_mode->hts_def *
								imx678->cur_vts * 1000000,
								imx678->pclk);
					if (frame_us < 1000)
						frame_us = 1000;
					usleep_range(frame_us, frame_us + 200);
			...
			const struct i2c_device_id *id)
{
	...
	if (eeprom_ctrl_node) {
		...
		if (IS_ERR_OR_NULL(eeprom_ctrl_client)) {
			dev_err(dev, "[imx678]can not get node\n");
			goto continue_probe;
		}
		eeprom_ctrl = i2c_get_clientdata(eeprom_ctrl_client);
		if (IS_ERR_OR_NULL(eeprom_ctrl)) {
			dev_err(dev, "[imx678]can not get eeprom i2c client\n");
		} else {

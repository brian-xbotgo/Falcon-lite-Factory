...
#define RK_UART_SFE			0x26 /* UART Shoadow FIFO Register */
#define RK_UART_HTX			0x29 /* UART Halt TX Register */
...
static void rk_serial_debug_init(struct platform_device *dev, void __iomem *base, phys_addr_t phy_base,
...
static int __init early_parse_fiq_enable(char *str)
{
    if (str) {
        if (strcmp(str, "1") == 0) {
            fiq_enabled = 1;
...
early_param("fiq_enable", early_parse_fiq_enable);
...
static int rk_fiq_debugger_enable(struct device *dev)
{
	struct rk_fiq_debugger *t = dev_get_drvdata(dev);

	pr_info("fiq debugger enable\n");
	console_lock();

	/* reset uart */
	rk_fiq_write(t, 0x0, UART_SRR);
	udelay(10);
	rk_fiq_write(t, 0x0, UART_MCR);
	rk_fiq_write(t, 0x1, UART_IER);
	rk_fiq_write(t, 0x0, RK_UART_HTX);
	rk_fiq_write(t, 0x7, UART_FCR);
	rk_fiq_write(t, 0x1, RK_UART_SFE);

	console_unlock();
	return 0;
}

static int rk_fiq_debugger_disable(struct device *dev)
{
	struct rk_fiq_debugger *t = dev_get_drvdata(dev);

	pr_info("fiq debugger disable\n");
	console_lock();

	rk_fiq_write(t, 0x0, UART_FCR);
	rk_fiq_write(t, 0x0, RK_UART_SFE);
	rk_fiq_write(t, 0x0, RK_UART_HTX);
	rk_fiq_write(t, 0x0, UART_IER);
	rk_fiq_write(t, 0x10, UART_MCR);
	rk_fiq_write(t, 0x6, UART_SRR);
                ...
                const char *buf, size_t count)
{
	...
	if (enable == fiq_enabled)
		return count;

	fiq_enabled = enable;

	if (enable)
		rk_fiq_debugger_enable(dev);
	else
		rk_fiq_debugger_disable(dev);
...
static struct attribute *rk_fiqdbg_attrs[] = {
	&dev_attr_enable.attr,
	NULL,
};
ATTRIBUTE_GROUPS(rk_fiqdbg);
...
static int __init rk_fiqdbg_probe(struct platform_device *pdev)
{
	...
	base = of_iomap(np, 0);
	if (base)
		rk_serial_debug_init(pdev, base, phy_base,
				     irq, signal_irq, wake_irq, baudrate);

	if (!fiq_enabled)
		rk_fiq_debugger_disable(&pdev->dev);
	return 0;
}

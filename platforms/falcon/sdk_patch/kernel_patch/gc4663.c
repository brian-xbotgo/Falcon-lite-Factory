...
static const struct regval gc4663_linear10bit_2560x1440_regs[] = {
	...
	{0x0324, 0xc8},
	{0x0325, 0x06},
	{0x0326, 0x6c}, // Harry :0x60 for 27MHZ
	{0x0327, 0x03},
	{0x0334, 0x40},
	{0x0336, 0x6c}, // Harry :0x60 for 27MHZ
	{0x0337, 0x82},
	{0x0315, 0x25},
	{0x031c, 0xc6},
	{0x0287, 0x18},
...
static void gc4663_get_otp(struct otp_info *otp, struct rkmodule_inf *inf)
{
	/* awb */
...
{
	struct otp_info *otp = gc4663->otp;
			...
			const struct i2c_device_id *id)
{
	...
	if (eeprom_ctrl_node) {
		...
		if (IS_ERR_OR_NULL(eeprom_ctrl_client)) {
			dev_err(dev, "[gc4663] can not get node\n");
			goto continue_probe;
		}
		eeprom_ctrl = i2c_get_clientdata(eeprom_ctrl_client);
		if (IS_ERR_OR_NULL(eeprom_ctrl)) {
			dev_err(dev, "[gc4663] can not get eeprom i2c client\n");
		} else {

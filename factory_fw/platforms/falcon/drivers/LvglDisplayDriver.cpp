...
static constexpr int FONT_LABEL_SIZE    = 48;
static constexpr int FONT_TIMER_SIZE    = 52;

static constexpr int STATUS_X = 152, STATUS_Y = 20;
static constexpr int KEY_X    = 10,  BAT_X    = 152, MODEL_X = 310, ROW2_Y = 60;
static constexpr int TIMER_X  = 140, TIMER_Y  = 100;
...
static void backlight_on()  {
    int fd = open("/sys/class/gpio/gpio92/value", O_WRONLY | O_SYNC);
    if (fd < 0) return;
    ::write(fd, "1", 1);
    close(fd);
}
...
// ---- Timer callbacks ----
void LvglDisplayDriver::bgTimerCb(lv_timer_t* /*t*/) {
...
void LvglDisplayDriver::updateTimerCb(lv_timer_t* t) {
    auto* self = static_cast<LvglDisplayDriver*>(t->user_data);
    if (!self) return;
    ...
    if (!model_read && self->model_label_) {
        ...
        if (!m.empty()) {
            ...
            if (!m.empty()) {
                lv_label_set_text(self->model_label_, m.c_str());
                model_read = true;
            }
        }
    }
    ...
    if (!content.empty()) {
        ...
        if (state >= 0 && state <= 4 && state != last_state) {
            last_state = state;
            switch (state) {
                case 0: lv_label_set_text(self->status_label_, ""); break;
                case 1: timer_running = false; lv_label_set_text(self->status_label_, "FINISHED"); break;
                case 2: {
                    struct timeval tv;
                    gettimeofday(&tv, nullptr);
                    aging_start_ms = (uint64_t)tv.tv_sec * 1000 + tv.tv_usec / 1000;
                    timer_running = true;
                    std::string conf = read_file("/userdata/aging_time.conf");
                    if (!conf.empty()) {
                        while (!conf.empty() && (conf.back() == '\n' || conf.back() == '\r')) conf.pop_back();
                    }
                    char text[64];
                ...
                case 4: timer_running = false; lv_label_set_text(self->status_label_, "AGING ABORT"); break;
            }
        }
    }
...
bool LvglDisplayDriver::init() {
    ...
    if (fbdev_init() != 0) {
        std::fprintf(stderr, "[LvglDisplay] fbdev_init failed\n");
        return false;
    }
    ...
    if (!label_font_ || !timer_font_) {
        std::fprintf(stderr, "[LvglDisplay] TTF font load failed (path=%s)\n", FONT_PATH);
    }
    ...
    bg_timer_     = lv_timer_create(bgTimerCb,     1000, nullptr);
...
uint32_t LvglDisplayDriver::taskHandler() {
    return lv_task_handler();
}

void LvglDisplayDriver::setBatteryPercent(int pct) {
    if (battery_label_) lv_label_set_text_fmt(battery_label_, "%d%%", pct);
}

void LvglDisplayDriver::setBatteryModel(const char* model) {
    if (model_label_) lv_label_set_text(model_label_, model);
}

void LvglDisplayDriver::setKeyValid(bool valid) {

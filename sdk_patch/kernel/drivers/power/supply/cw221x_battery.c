...
#define USER_RSENSE			(10 * 1000)  // 根据硬件参数设置
...
static int i2c_battery_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct battery_data *battery_dat = &battery_dev->cw_bat->battery_dat;
	struct cw_battery *cw_bat = battery_dev->cw_bat;


    if(_IOC_TYPE(cmd) != I2CBAT_BASE)
	{
		cw_printk("i2c ioctl cmd error\n");
        return -ENOTTY;
	}
	...
	switch(cmd)
	{
		case I2CBAT_GET_STATUS:
			battery_dat->capacity = cw_bat->ui_soc;
			battery_dat->present  = cw_bat->voltage <= 0 ? 0 : 1;
			battery_dat->voltage_now = cw_bat->voltage * 1000;
			battery_dat->technology = "Li-ion";
			battery_dat->temp = cw_bat->temp / 10;
			battery_dat->charge_full = 1;  //cw_bat->design_capacity * CAPACITY_UNIT;
			battery_dat->charge_counter = 0; //CHARGE_COUNTER;
			battery_dat->capacity_level = cw_bat->ui_soc > 0 ? 1 : 0;
			battery_dat->current_now = cw_bat->cw_current;
			battery_dat->health = POWER_SUPPLY_HEALTH_GOOD;
			battery_dat->cycle_count = cw_bat->cycle;
			battery_dat->charger_status = power_supply_is_system_supplied() ? true : false;
			battery_dat->battery_status = 1;
		...
		case I2CBAT_GET_VBUS_STATUS:
			{
				int vbus_online = power_supply_is_system_supplied() ? 1 : 0;

				//om_printk("get vbus status %d\n", vbus_online);
				if (copy_to_user(argp, &vbus_online, sizeof(int))) {
					printk("ioctl I2CBAT_GET_VBUS_STATUS failed\n");
					return -EFAULT;
				}
			}
			break;
		case I2CBAT_GET_CHARGE_STATUS:
			{
				int charge_status = power_supply_is_system_supplied() ? 1 : 0;

				//printk("get charge status %d\n", charge_status);
				if (copy_to_user(argp, &charge_status, sizeof(int))) {
					printk("ioctl I2CBAT_GET_CHARGE_STATUS failed\n");
					return -EFAULT;
				}
			}
			break;
...
static int cw221X_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mutex_init(&cw_bat->battery_dev.ibe_lock);
	mutex_init(&cw_bat->battery_dev.i2c_lock);
	...
	cw_bat->battery_dev.name = CW_PROPERTIES;
	cw_bat->battery_dev.cw_bat = cw_bat;
	
	alloc_chrdev_region(&cw_bat->battery_dev.devnu, 0, 32, cw_bat->battery_dev.name);
	cdev_init(&cw_bat->battery_dev.cdev, &i2c_battery_fops);
	cdev_add(&cw_bat->battery_dev.cdev, cw_bat->battery_dev.devnu, 256);

	cw_bat->battery_dev.cdev_class = class_create(THIS_MODULE, cw_bat->battery_dev.name);
	if(!device_create(cw_bat->battery_dev.cdev_class, NULL, cw_bat->battery_dev.devnu, NULL, cw_bat->battery_dev.name))
	{
...
static void cw221X_remove(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "remove: cdev_del cdev=%p\n", &cw_bat->battery_dev.cdev);
	cdev_del(&cw_bat->battery_dev.cdev);

	dev_info(&client->dev, "remove: device_destroy class=%p dev=0x%x\n",
			 cw_bat->battery_dev.cdev_class, cw_bat->battery_dev.devnu);
	device_destroy(cw_bat->battery_dev.cdev_class, cw_bat->battery_dev.devnu);

	dev_info(&client->dev, "remove: class_destroy class=%p\n", cw_bat->battery_dev.cdev_class);
	class_destroy(cw_bat->battery_dev.cdev_class);

	dev_info(&client->dev, "remove: unregister_chrdev_region dev=0x%x count=%d\n",
			 cw_bat->battery_dev.devnu, 32);
	unregister_chrdev_region(cw_bat->battery_dev.devnu, 32);
}

...
{
	dev_info(sc89890->dev, "F_CHG_CFG enable\n");
					     ...
					     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		dev_info(sc89890->dev, "POWER_SUPPLY_PROP_ONLINE %d\n", val->intval);
		if (val->intval) {
...
static int sc89890_init_param(struct sc89890_device *sc89890)
{
	int ret = 0;

	// Fast Charge Current (3A)
	ret = sc89890_field_write(sc89890, F_ICHG, 0x32);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_ICHG failed %d\n", ret);
		return ret;
	}

	// Charge Voltage (4.2V)
	ret = sc89890_field_write(sc89890, F_VREG, 0x16);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_ICHG failed %d\n", ret);
		return ret;
	}

	ret = sc89890_field_write(sc89890, F_BATLOWV, 0x01);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_ICHG failed %d\n", ret);
		return ret;
	}

	ret = sc89890_field_write(sc89890, F_VRECHG, 0x01);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_ICHG failed %d\n", ret);
		return ret;
	}
	...
	ret = sc89890_field_write(sc89890, F_CONV_START, 0x01);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_ICHG failed %d\n", ret);
		return ret;
	}
	...
	ret = sc89890_field_write(sc89890, F_CONV_RATE, 0x01);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_ICHG failed %d\n", ret);
		return ret;
	}
	...
	// Input current limit
	ret = sc89890_field_write(sc89890, F_IILIM, 0x2F);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_ICHG failed %d\n", ret);
		return ret;
	}
	...
	// set VSYS_MIN (3.7V)
	ret = sc89890_field_write(sc89890, F_SYSVMIN, 0x07);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_SYSVMIN failed %d\n", ret);
		return ret;
	}

	// OTG Disable
	ret = sc89890_field_write(sc89890, F_OTG_CFG, 0x00);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_OTG_CFG failed %d\n", ret);
		return ret;
	}

	// Charge Enable
	ret = sc89890_field_write(sc89890, F_CHG_CFG, 0x01);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_CHG_CFG failed %d\n", ret);
		return ret;
	}
...
static void sc89890_shutdown(struct i2c_client *client)
{
	struct sc89890_device *sc89890 = i2c_get_clientdata(client);
	int ret = 0;

	dev_info(sc89890->dev, "%s\n", __func__);

	ret = sc89890_field_write(sc89890, F_BATFET_DIS, 0x1);
	if (ret < 0) {
		dev_err(sc89890->dev, "Config F_BATFET_DIS failed %d\n", ret);
	}
}

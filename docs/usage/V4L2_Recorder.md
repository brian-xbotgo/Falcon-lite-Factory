...
| 文件 | 职责 |
|------|------|
| `include/platforms/common/interface/IRecorder.h` | 录像接口抽象 (start/stop/isRecording)、CameraConfig/RecorderConfig 数据结构 |
| `include/control/V4l2Recorder.h` | 通用 V4L2 采集 + MP4 封装，编码器通过 `IEncoder` 工厂注入 |
| `include/platforms/common/interface/IEncoder.h` | 编码器接口抽象 (init/encode/deinit + 码率配置) |
| `platforms/falcon/drivers/RkMppEncoder.h` | FALCON 平台 MPP H.264 硬编码封装 |
| `include/control/RecorderController.h` | 控制器，持有 IRecorder 指针，解耦测试层与实现层 |
| `src/control/V4l2Recorder.cpp` | V4L2 操作实现 (open/format/mmap/QBUF/DQBUF/STREAMON/STREAMOFF) |
| `platforms/falcon/drivers/RkMppEncoder.cpp` | MPP 编码实现 (mpp_create / mpp_enc_cfg / encode_put_frame / encode_get_packet) |
| `src/control/RecorderController.cpp` | 控制器实现 |
| `src/platforms/common/BaseRecorder.cpp` | 基础平台兜底录像实现 (stub) |
...
```
IRecorder (接口)
  ├── BaseRecorder     ← 基础平台回退 (无操作)
  └── Rk3576Recorder   ← FALCON 平台录像 driver
          └── V4l2Recorder     ← 通用 V4L2 采集和 MP4 封装
          ├── CameraWorker[]
          ...
          │      ├── mplane  (MPLANE 标志位)
          │      └── encoder (IEncoder, 由平台注入；FALCON 使用 RkMppEncoder)
          └── m_cfg          (RecorderConfig)

RecorderController (单例)
  └── m_recorder: IRecorder*  → 委托给平台 recorder 或 BaseRecorder
```
